...
export default function SolarPanelSearch() {
    ...
    useEffect(() => {
        ...
        getSolarPanels(MOCK_IDS).then((result) => { if (active) { setItems(result) } })
        ...
        <div className="flex flex-col gap-8">
            ...
            <div className="flex flex-row flex-wrap items-center gap-4">
                ...
                {FILTER_CHIPS.map((chip) => (
                    <span key={chip} className="rounded-full bg-input-bg px-4 py-2 font-medium text-black/70">{chip}</span>
                ))}
            </div>

            <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-4">
                {items.map((item) => (
                    <Link key={item.id} to={`/produto/${item.id}`} className="flex flex-col gap-2">
                        <img src={item.photos.heroImage.url} alt={item.photos.heroImage.description} className="aspect-square w-full rounded-medium bg-input-bg object-cover" />
