...
import type { SolarPanelFeedSummary } from '@/domain/models/announcemnt/solarPanel'
...
const items: SolarPanelFeedSummary[] = [
  {
    ...
    unitPrice: 200,
    photos: { heroImage: { description: 'Placa solar 1', url: 'https://example.com/1.png' }, otherImages: [] },
  },
  {
    id: '2',
    title: 'Painel Solar Monocristalino 517W',
    unitPrice: 1432.6,
    photos: { heroImage: { description: 'Placa solar 2', url: 'https://example.com/2.png' }, otherImages: [] },
  },
]
...
describe('Corridor', () => {
  it('renders the title', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Principais placas solares presentes no mercado' })).toBeInTheDocument()
  })
  ...
  it('renders one card per item, with title, price and a link to the product page', () => {
    ...
    expect(screen.getByRole('link', { name: /Painel Solar Monocristalino 517W/ })).toHaveAttribute('href', '/produto/2')
  })
  ...
  it('defaults the action label and calls onActionClick when it is clicked', () => {
    ...
    const action = screen.getByRole('button', { name: 'acessar todas as peças' })
    action.click()
