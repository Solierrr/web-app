 ...
 */
export default function ProfileOnboarding({ kind = "user", name = "", onComplete }: ProfileOnboardingProps) {
  ...
  return (
    <ProfilePage bannerUrl={bannerUrl || undefined} avatarUrl={avatarUrl || undefined} name={name || t("previewFallbackName")}>
      <div className="flex max-w-md flex-col gap-6">
        <div className="flex flex-col gap-2">
          <h2>{t(`${scope}.bannerHeading`)}</h2>
          <Input name="bannerUrl" placeholder={t("bannerPlaceholder")} value={bannerUrl} onChange={(event) => setBannerUrl(event.target.value)} />
        </div>

        <div className="flex flex-col gap-2">
          <h2>{t(`${scope}.avatarHeading`)}</h2>
          <Input name="avatarUrl" placeholder={t("avatarPlaceholder")} value={avatarUrl} onChange={(event) => setAvatarUrl(event.target.value)} />
        </div>

        <div className="flex flex-row gap-2">
          <PrimaryButton content={t("complete")} description={t("completeDescription")} onClick={handleComplete} rounded />
          <SecondaryButton content={t("skip")} description={t("skipDescription")} onClick={handleComplete} rounded />
        </div>
      </div>
    </ProfilePage>
  );
